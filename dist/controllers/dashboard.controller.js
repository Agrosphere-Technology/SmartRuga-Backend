...
exports.getRanchDashboard = getRanchDashboard;
const http_status_codes_1 = require("http-status-codes");
const sequelize_1 = require("sequelize");
const models_1 = require("../models");
const dashboard_helpers_1 = require("../helpers/dashboard.helpers");
const apiResponse_1 = require("../utils/apiResponse");
async function getRanchDashboard(req, res) {
    try {
        ...
        const dueSoonDays = Number.isFinite(parsedDueSoonDays) && parsedDueSoonDays > 0
            ? Math.min(parsedDueSoonDays, 30)
            : 7;
        const now = new Date();
        const todayStart = (0, dashboard_helpers_1.startOfDay)(now);
        const todayEnd = (0, dashboard_helpers_1.endOfDay)(now);
        const dueSoonEnd = (0, dashboard_helpers_1.endOfDay)((0, dashboard_helpers_1.addDays)(now, dueSoonDays));
        const animalStatsPromise = models_1.sequelize.query(`
            SELECT
                COUNT(*)::text AS total,
        ...
        const vaccinationRowsPromise = models_1.Vaccination.findAll({
            where: {
                ranch_id: ranchId,
                deleted_at: null,
                next_due_at: {
        ...
        const inventoryStatsPromise = models_1.sequelize.query(`
            SELECT
                COUNT(*)::text AS total,
        ...
        const [animalStatsResult, taskStatsResult, submissionApprovalStatsResult, inventoryStatsResult, concernStatsResult, recentAssignedConcerns, recentActivityRows, vaccinationRows,] = await Promise.all([
            animalStatsPromise,
            taskStatsPromise,
        ...
        const recentActivity = (0, dashboard_helpers_1.buildDashboardActivity)(recentActivityRows);
